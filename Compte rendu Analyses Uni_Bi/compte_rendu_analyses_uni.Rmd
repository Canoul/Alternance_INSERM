...
Chaque enregistrement correpond à un patient qui a réalisé une ponction lombère afin de recueillir le liquide Céphalo-Rachidien (LCR).  
Une ponction lombère n'étant pas réalisée pour des raisons mineurs, tous les patients de la BDD ont présentés des problèmes cognitifs.

# Objectif

L'objectif de l'étude est d'analyser l’association entre différents biomarqueurs sanguins et les protéines Abeta42, Abeta40, Tau et ptau, marqueurs de la maladie d’Alzheimer. 


```{r , echo = FALSE, message = FALSE, warning = FALSE}
#Importation
#data = read.csv2("//172.27.137.244/g_boilay/alternance/BASE_EPIDATA.csv")
#data = read.csv2("C:/Users/g_boilay/Documents/BASE_EPIDATA.csv")
data = read.csv2("//172.27.137.244/g_boilay/alternance/table/Masque_LCR_Novembre 2022.csv")

#data = read.csv2("C:/Users/edday/Documents/3ème année/alternance/BASE_EPIDATA.csv")
```
...
```{r , echo = FALSE, message = FALSE, warning = FALSE}
di = matrix(nrow = 1, ncol = length(data))
di = as.data.frame(di)
...
for (i in 1:1){
  j = 1
  while(is.na(data[j,i]) == T) {
    j = j + 1
    if(data[j-1,i] == "TASSIGNON" & is.na(data[j-1,i]) == T){
      next
    }
    print(j)
    if(j >= 1000){
...
```{r, echo = FALSE}
data2$sexe.reg[data2$sexe == 1] = FALSE
data2$sexe.reg[data2$sexe == 2] = TRUE

data2$sexe.reg2[data2$sexe == 1] = "Homme"
data2$sexe.reg2[data2$sexe == 2] = "Femme"

p.table(data2$sexe.reg2)

data2$sexe = data2$sexe.reg
```
...
non scolarisé                        =      niveau études mineurs   
primaire sans diplôme                =      niveau études mineurs   
certificat d'études                  =      niveau études mineurs   
secondaire (collège/CAP/brevet)      =      niveau études intermédiaires   
secondaire (BEP/lycée)               =      niveau études intermédiaires  
baccalauréat                         =      niveau études intermédiaires  
études supérieures                   =      études supérieurs  
non renseigné                        =      NA  
...
```{r , echo = FALSE}
data2$tabac.reg[data2$tabac == 1] = "Non Fumeur"
data2$tabac.reg[data2$tabac == 2] = "Fumeur Actif"
data2$tabac.reg[data2$tabac == 3] = "Fumeur severe"
data2$tabac.reg[data2$tabac == 4] = "Fumeur Occasionnelle"
...
```{r , echo = FALSE, message = FALSE, warning = FALSE, results = FALSE}
data2 = data2 %>%
  mutate(poids = replace(poids,poids >= 200 ,NA))
...
data2$imc.reg[data2$imc >= 40] = "Obésité"
data2$imc.reg[data2$imc <= 40] = "Obésité"
data2$imc.reg[data2$imc <= 35] = "Obésité"
...
data2$imc.reg2[data2$imc >= 40] = "Obésité massive"
data2$imc.reg2[data2$imc <= 40] = "Obésité sévère"
data2$imc.reg2[data2$imc <= 35] = "Obésité modérée"
data2$imc.reg2[data2$imc <= 30] = "Surpoids"
data2$imc.reg2[data2$imc <= 25] = "Corpulence normale"
data2$imc.reg2[data2$imc <= 18.5] = "Insuffisance pondérale"

p.table(data2$imc.reg2)
```
#### Barplot

```{r,echo=FALSE, warning = FALSE}
data2$imc.reg <- fct_relevel(data2$imc.reg,c("Insuffisance pondérale","Corpulence normale","Surpoids","Obésité modérée","Obésité sévère","Obésité morbide ou massive"))
...
### Hypertension Artérielle {.tabset}
...
#### Liste des médicaments {.tabset}

##### Médicament contre le cholestérol

```{r, echo = FALSE}
medi_chol = c("Atorvastatine","Crestor", "pravastatine", "SIMVASTATINE","Tahor","Vasten","Zocor","Lipanthyl")
...
```{r, echo = FALSE}
medi_HTA = c("Aldactone","esidrex","fludex","Hydrochlorothiazide","spironolactone","aldactazine","amlor","aprovel","Amlodipine","lercan","ramipril","coversyl","cotareg","triatec","lasilix","atenolol","coaprovel","isoptine","temerit")
...
medi_diabete = c("glucophag","metformine ","stagid","diamicron")
...
medi_ma = c("donepezil","Rivastigmine","risperdal","tanakan","exelon","rivastigmine patch
")
```
...
medi_depression = c("seroplex","deroxat","Paroxétine","prozac","effexor","lamictal","athymil","mianserine","fluoxetine","escitalopram","laroxyl","sertraline","zoloft","seropram")
```
...
medi_vitamines = c("uvedose","vitamine")
```
...
medi_anxiete = c("seresta","imovane","lexomil","lysanxia","seroplex")
```
...
medi_antidouleur = c("doliprane","aspegic","paracetamole","aspirine","ixprim","dafalgan","tramadol","cortancyl")
...
medi_trouble = c("depakote","lamictal")
```
...
medi_sommeil = c("melatonin","mianserine","xanax","atarax","alprazolam","stilnox","temesta","circadin")
```
...
for(i in 1:nrow(data2)){
  ...
  if (is.na(cardiovasculaire[i]) == T){cardiovasculaire[i] = 0}
  ...
  if (is.na(trouble_comportement[i]) == T){trouble_comportement[i] = 0}
  if (is.na(vitamines[i]) == T){vitamines[i] = 0}
}


data2 = cbind(data2,ch,hta_med,dia,antidouleur,anxiete,cardiovasculaire,depression,alzheimer,parkinson,sommeil,trouble_comportement,vitamines)
```
...
for (i in 1:length(L)){
  data2[L[i]] = as.Date(eval(parse(text = paste0("data2$",L[i]))),format = "%d/%m/%Y")
}
...
for (i in 1:nrow(data2)){
  li = c()
    for (j in 1:length(L)){
      li[j] = difftime(data2[i,L[j]],data2$datepl[i],unit="weeks")/52.25
    }
  mmse_nb[i] = 20 - sum(is.na(li))
  if (sum(is.na(li)) < 20){
  ...
  mmse_first[i] = abs(li[mn])
  mmse_last[i] = abs(li[mx])
...
data2 = cbind(data2,mmse,mmse_first,mmse_last,mmse_diff,mmse_avant_pl,mmse_apres_pl,mmse_nb)
```


### Mesures de MMSE {.tabset}

#### Nombre de mesures {.tabset}

##### Summary
```{r , echo = FALSE}
summary(as.factor(data2$mmse_nb))
```

##### Barplot

```{r,echo=FALSE, warning = FALSE}
bar(as.factor(data2$mmse_nb))
```

#### Temps entre la date de la première mesure et la date de la dernière mesures (en année) {.tabset}
...
##### Histogramme
```{r , echo = FALSE}
hist(data2$mmse_diff,main = "Distribution du temps entre la 1ère et la dernière mesure",xlab = "Temps (en année)")
```

 
### Mesures du MMSE avant la PL {.tabset}

#### Nombre de mesures {.tabset}

##### Summary
```{r , echo = FALSE}
summary(as.factor(data2$mmse_avant_pl))
```

##### Barplot

```{r,echo=FALSE, warning = FALSE}
bar(as.factor(data2$mmse_avant_pl))
```


#### Temps entre la date de la première mesure et la PL (en année) {.tabset}

##### Summary
```{r , echo = FALSE}
summ(data2$mmse_first)
```

##### Boxplot
```{r , echo = FALSE}
boxplot(data2$mmse_first)
```

##### Histogramme
```{r , echo = FALSE}
hist(data2$mmse_first,main = "Distribution du temps entre la 1ère mesure et la ponction lombère",xlab = "Temps (en année)")
```
...
### Mesures du MMSE après la PL {.tabset}
...
##### Summary
```{r , echo = FALSE}
summary(as.factor(data2$mmse_apres_pl))
```

##### Barplot

```{r,echo=FALSE, warning = FALSE}
bar(as.factor(data2$mmse_apres_pl))
```

#### Temps entre la date de la dernière mesure et la PL (en année) {.tabset}
...
##### Histogramme
```{r , echo = FALSE}
hist(data2$mmse_last,main = "Distribution du temps entre la dernière mesure et la ponction lombère",xlab = "Temps (en année)")
```
...
```{r,echo=FALSE}
data2$ma = 0
data2 = data2 %>% mutate(ma = replace(ma,diagnostic_cli == 1,1))

data2$ma_mci = 0
data2 = data2 %>% mutate(ma_mci = replace(ma_mci,diagnostic_cli == 1 | diagnostic_cli == 2,1))
...
![](C:/Users/g_boilay/Documents/Seuil.png)
...
```{r , echo = FALSE, message = FALSE, warning = FALSE}
data2$abeta40 = as.numeric(data2$abeta40)

data2$ab42_ab40 = FALSE

data2$ab42_ab40 = data2$abeta42/data2$abeta40
...
for (i in 1:nrow(data2)){
  ...
  }else if (is.na(data2$abeta42[i]) == TRUE){
      abeta42t[i] = NA
...
for (i in 1:nrow(data2)){
   ...
   if (between(data2$datepl[i],as.Date("01/01/2008", format = "%d/%m/%Y") ,as.Date("05/12/2012",format = "%d/%m/%Y",)) & data2$ab42_ab40[i] > 0.071 & is.na(data2$ab42_ab40[i]) == FALSE){
    ab42_ab40t[i] = "ab42_ab40 normal"

    
  }else if (between(data2$datepl[i],as.Date("05/12/2012", format = "%d/%m/%Y"),as.Date("20/05/2016",format = "%d/%m/%Y",)) & data2$ab42_ab40[i] > 0.065 & is.na(data2$ab42_ab40[i]) == FALSE){
    ab42_ab40t[i] = "ab42_ab40 normal"

    
  }else if (between(data2$datepl[i],as.Date("20/05/2016", format = "%d/%m/%Y"),as.Date("24/05/2018",format = "%d/%m/%Y",)) & data2$ab42_ab40[i] > 0.083 & is.na(data2$ab42_ab40[i]) == FALSE){
    ab42_ab40t[i] = "ab42_ab40 normal"
  
    
  }else if (data2$datepl[i] >= as.Date("24/05/2018",format = "%d/%m/%Y") & data2$ab42_ab40[i] > 0.083 & is.na(data2$ab42_ab40[i]) == FALSE){
      ab42_ab40t[i] = "ab42_ab40 normal"

      
  }else if (is.na(data2$ab42_ab40[i]) == TRUE){
      ab42_ab40t[i] = NA

      
  }else{
    ab42_ab40t[i] = "ab42_ab40 faible"
 
    
  }
}

data2 = cbind(data2,ab42_ab40t)
```
...
valeur brute multiplié par -1 pour qu’une augmentation soit associé à un risque
...
```{r,echo = FALSE}
data2 = data2 %>% group_by(date_interval) %>% mutate(ab42_ab40 = scale(log(ab42_ab40))*-1) %>% ungroup()
...
#### Boxplot
```{r , echo = FALSE}
boxplot(data2$ab42_ab40)
```

#### Histogramme
```{r , echo = FALSE}
hist(data2$ab42_ab40)
```
...
Standardisation du logarithme de la variable par -1 en prenant en compte les différentes périodes
...
data_biomarqueurs_pearson = data2 %>%
  select (abeta42,abeta40,tau,ptau, age,mmse, apoe.reg2) %>% 
  na.omit()
...
paste0("N = ",nrow(data_biomarqueurs_pearson))
...
paste0("N = ",nrow(data_biomarqueurs_spearman))
...
#### Summary
```{r , echo = FALSE}
data2$cholesterol_total = as.numeric(data2$cholesterol_total)
...
#### Summary
```{r , echo = FALSE}
data2$cholesterol_hdl = as.numeric(data2$cholesterol_hdl)
...
Cholesterol_hdl faible <  0.4 g/l  
Cholesterol_hdl élevé  >  0.6 g/l  
```{r , echo = FALSE,warning=FALSE}
...
hypocholestérolémie <  0.9 g/l  
hypercholestérolémie  >  1.6 g/l  
```{r , echo = FALSE,warning=FALSE}
data2$cholesterol_hdl.reg[data2$cholesterol_hdl < 0.9] = "hypocholestérolémie"
data2$cholesterol_hdl.reg[data2$cholesterol_hdl >= 0.9] = "cholesterol_hdl normal"
data2$cholesterol_hdl.reg[data2$cholesterol_hdl > 1.6] = "hypercholestérolémie"
...
#### Summary
```{r , echo = FALSE}
data2$cholesterol_ldl = as.numeric(data2$cholesterol_ldl)
...
#### Summary
```{r , echo = FALSE}
data2$glucose_jeun = as.numeric(data2$glucose_jeun)
...
Hypoglycémie <= 3.9 nmol/l  
Hyperglycémie > 6.1 nmol/l
...
Les valeurs considérées comme aberrantes ont été retirées 

```{r , echo = FALSE}
data2$triglycerides = as.numeric(data2$triglycerides)
data2 = data2 %>%
  mutate(triglycerides = replace(triglycerides,triglycerides >= 15 ,NA))
...
paste0("N = ",nrow(data_bio_sanguin_age_pearson))
...
paste0("N = ",nrow(data_bio_sanguin_age_spearman))
...
Carence en Vitamine B12 < 147.6 pmol/l
...
Carence en vitamine D < 50 nmol/l  
...
```{r , echo = FALSE}
data2$proteines = outlier_hampel(data2$proteines)
...
Hyperthyroïdie < 0.5 mu/l
Hyperthyroïdie > 5 mu/l
...
Urée faible  <  3 nmol/l                 
Urémie chronique ou aiguë  > 7.5 nmol/l
...
créatinine faible < 53 umol/l
créatinine élevé  > 97 umol/l
...
Calcium faible <= 2.2 mmol/l
Calcium élevé > 5.6 mmol/l
...
paste0("N = ",nrow(autres_marqueurs_age_pearson))
...
paste0("N = ",nrow(autres_marqueurs_age_spearman))
...
#### Autres marqueurs + Biomarqueur MA {.tabset}

##### Pearson

```{r,echo=FALSE}
autres_marqueurs_age_pearson = data2 %>%
  select (folates,vitamine_b12,vitamine_d,proteines, tsh, uree,creatinine,calcium,abeta42,abeta40,tau,ptau) %>% 
  na.omit()
  ...
  if (len_x == 1){
    ...
    tbl = tbl_regression(reg,
                        label = list(`eval(parse(text = paste0(x[1])))` ~ x[1],
                                     ...
                                     `eval(parse(text = paste0(x[8])))` ~ x[8],
                                     `eval(parse(text = paste0(x[9])))` ~ x[9],
                                     `eval(parse(text = paste0(x[10])))` ~ x[10]))
  }
...
Les marqueurs de la maladie d'Alzheimer (MA) sont les peptides amyloïdes Abeta42, Abeta42/Abeta40, et les protéines tau totale (tau) et tau phosporylée (ptau).
<br>
Les biomarqueurs sanguins d’intérêt : Cholesterol (total, HDL, LDL), glucose à jeun et triglycérides. Les mesures sont standardisées pour permettre la comparaison de leurs effets.
<br>

On s’intéresse à l’association entre les différents biomarqueurs sanguins d’intérêt et les marqueurs de la MA en prenant en compte les différents facteurs d’ajustement suivant :  
- l'âge (en dizaine d'année)  
- le sexe (Homme=0 et femme =1)  
- la présence d'allèle E4 (0,1,2)  
- le score MMSE le plus proche de la PL  
- la prise de médicament contre le cholestérol (0,1)  
- la prise de médicament contre la haute tension artérielle (0,1)  
- la prise de médicament contre le diabète (0,1)  
- l'IMC  
- Niveau d'étude  
<br>
Pour chaque marqueur de la MA, vous trouverez :  
-	Un tableau de fréquence  
-	L’association avec chacun des biomarqueurs sanguins d’intérêt en continu  
-	L’association avec chacun des biomarqueurs sanguins d’intérêt en dichotomique  
...
Description des caractéristiques des biomarqueurs sanguins d’intérêt et des covariables en fonction du niveau normal ou anormal des biomarqueurs de la MA. Le tableau de fréquence est basé sur les individus sans valeur manquante dans la variable du marqueur de la MA et des covariables.
...
Les valeurs du marqueur de la MA sont transformées par le logarithme puis standardisées en fonction des différentes périodes de la PL. Les valeurs des variables Abeta42 et Abeta40 sont multipliés par -1 pour qu’une augmentation soit associée à un risque.
Les modèles sont basés sur une régression linéaire.	
...
En se basant sur les seuils en fonction des différentes périodes, on formate les variables des marqueurs de la MA en dichotomique, avec 0 pour les valeurs normales et 1 pour les valeurs anormales dépassant les seuils, excepté pour le ratio Abeta42/Abeta40, pour lequel nous étudions seulement en continu
<br>

Les modèles sont basés sur une régression logistique.
<br>

Les analyses sont réalisées sur les individus sans valeur manquante pour les marqueurs de la MA, les covariables et le biomarqueur sanguin concerné (population différente selon le biomarqueur sanguin étudié).  Les modèles sont ajustés de manière successive sur les covariables de la manière suivante :

-	Modèle 1 :  âge  
-	Modèle 2 :  âge, sexe, niveau d’étude  
-	Modèle 3 :  âge, sexe, niveau d’étude, APOE  
-	Modèle 4 :  âge, sexe, niveau d’étude, APOE, MMSE  
-	Modèle 5 :  âge, sexe, niveau d’étude, APOE, MMSE, les traitements  
-	Modèle 6 :  âge, sexe, niveau d’étude, APOE, MMSE, les traitements, IMC  
<br>

Puis un tableau récapitulatif des résultats des différents modèles.
...
data2$presence_biomarqueur = 0
data2 = data2 %>% mutate(presence_biomarqueur = replace(presence_biomarqueur,is.na(glucose_jeun) == F | is.na(cholesterol_total) == F | is.na(triglycerides) == F,1))
...
data_am$hta_med = as.factor(data_am$hta_med)
data_am$apoe.reg2 = as.factor(data_am$apoe.reg2)

labels(data_am)  <- c(age = 'Age', sexe_reg = "Femme", glucose_jeun = "Glucose",apoe.reg2 = "Présence E4", mmse = "MMSE", ch = "Traitement cholesterol", dia = "Traitement Diabete",triglycerides = "Triglycerides",cholesterol_total = "Cholesterol",cholesterol_hdl = "Cholesterol hdl",cholesterol_ldl = "Cholesterol ldl",niveau_etude_reg = "Niveau d'étude",cholesterol_total = "Cholesterol",glucose_jeun = "Glucose",triglycerides = "Triglycerides",hta_med = "Traitement Hypertenseur",imc_reg = "IMC")

tab1 <- tableby(abeta42t2~  age + sexe_reg  + cholesterol_total + glucose_jeun + triglycerides + apoe.reg2 + mmse + ch + dia + hta_med + niveau_etude_reg + imc_reg, data=data_am)
...
```{r,echo=FALSE}
x = c("cholesterol_total","age","sexe","niveau_etude_reg","apoe.reg2","cholesterol_total*apoe.reg2")
m3 = glm_reg(y,x,dt)
m3
...
```{r,echo=FALSE}
x = c("cholesterol_total","age","sexe","niveau_etude_reg","apoe.reg2","mmse","ch","dia","hta_med")
m5 = glm_reg(y,x,dt)
m5
```
...
```{r,echo=FALSE,warning=FALSE}
x = c("cholesterol_total","age","sexe","niveau_etude_reg","apoe.reg2","mmse","ch","dia","hta_med","imc_reg")
m6 = glm_reg(y,x,dt)
m6
```
...
```{r,echo=FALSE}
x = c("glucose_jeun","age","sexe","niveau_etude_reg","apoe.reg2","glucose_jeun*apoe.reg2")
m3 = glm_reg(y,x,dt)
m3
```
...
```{r,echo=FALSE}
x = c("glucose_jeun","age","sexe","niveau_etude_reg","apoe.reg2","mmse","ch","dia","hta_med")
m5 = glm_reg(y,x,dt)
m5
```
...
```{r,echo=FALSE,warning=FALSE}
x = c("glucose_jeun","age","sexe","niveau_etude_reg","apoe.reg2","mmse","ch","dia","hta_med","imc_reg")
m6 = glm_reg(y,x,dt)
m6
```
...
#### Association avec le triglycerides {.tabset}

```{r,echo=FALSE}
### Association avec le triglycerides 

data_am_triglycerides = filter(data2, is.na(abeta42t2) == F,
...
data_am_triglycerides$triglycerides = scale(log(data_am_triglycerides$triglycerides))
data_am_triglycerides$age = data_am_triglycerides$age/10

y = "abeta42t2"
dt = data_am_triglycerides
...
```{r,echo=FALSE}
x = c("triglycerides","age","sexe","niveau_etude_reg")
m2 = glm_reg(y,x,dt)
m2
```
...
```{r,echo=FALSE}
x = c("triglycerides","age","sexe","niveau_etude_reg","apoe.reg2","triglycerides*apoe.reg2")
m3 = glm_reg(y,x,dt)
m3
```
...
```{r,echo=FALSE}
x = c("triglycerides","age","sexe","niveau_etude_reg","apoe.reg2","mmse")
m4 = glm_reg(y,x,dt)
m4
```
...
```{r,echo=FALSE}
x = c("triglycerides","age","sexe","niveau_etude_reg","apoe.reg2","mmse","ch","dia","hta_med")
m5 = glm_reg(y,x,dt)
m5
```
...
```{r,echo=FALSE,warning=FALSE}
x = c("triglycerides","age","sexe","niveau_etude_reg","apoe.reg2","mmse","ch","dia","hta_med","imc_reg")
m6 = glm_reg(y,x,dt)
m6
```
...
data_ac_triglycerides = filter(data2, is.na(abeta42) == F,
               is.na(triglycerides) == F,
               is.na(age) == F,
               is.na(sexe) == F,
               is.na(apoe.reg2) == F,
...
data_ac_triglycerides$triglycerides = scale(log(data_ac_triglycerides$triglycerides))
data_ac_triglycerides$age = data_ac_triglycerides$age/10

y = "abeta42"
dt = data_ac_triglycerides
...
### Ratio Abeta42/Abeta40 en Continue {.tabset}

#### Association avec le cholestérol {.tabset}
```{r,echo=FALSE}
### Association avec cholestérol 

data_a40c_cho = filter(data2, is.na(ab42_ab40) == F,
               is.na(cholesterol_total) == F,
               is.na(age) == F,
               is.na(sexe) == F,
               is.na(apoe.reg2) == F,
...
data_a40c_glucose = filter(data2, is.na(ab42_ab40) == F,
               is.na(glucose_jeun) == F,
               is.na(age) == F,
               is.na(sexe) == F,
               is.na(apoe.reg2) == F,
...
data_a40c_triglycerides = filter(data2, is.na(ab42_ab40) == F,
               is.na(triglycerides) == F,
               is.na(age) == F,
               is.na(sexe) == F,
               is.na(apoe.reg2) == F,
...
data_a40c_triglycerides$triglycerides = scale(log(data_a40c_triglycerides$triglycerides))
data_a40c_triglycerides$age = data_a40c_triglycerides$age/10

y = "ab42_ab40"
dt = data_a40c_triglycerides
...
data_tm$hta_med = as.factor(data_tm$hta_med)
data_tm$apoe.reg2 = as.factor(data_tm$apoe.reg2)

labels(data_tm)  <- c(age = 'Age', sexe_reg = "Sexe", glucose_jeun = "Glucose",apoe.reg2 = "Présence E4", mmse = "MMSE", ch = "Traitement cholesterol", dia = "Traitement Diabete",triglycerides = "Triglycerides",cholesterol_total = "Cholesterol",cholesterol_hdl = "Cholesterol hdl",cholesterol_ldl = "Cholesterol ldl",niveau_etude_reg = "Niveau d'étude",cholesterol_total = "Cholesterol",hta_med = "Traitement Hypertenseur", imc_reg = "IMC")

tab1 <- tableby(taut2~  age + sexe_reg  + cholesterol_total + glucose_jeun + triglycerides + apoe.reg2 + mmse + ch + dia + hta_med + niveau_etude_reg + imc_reg, data=data_tm)
summary(tab1, text=TRUE)
```
...
data_tm_triglycerides = filter(data2, is.na(taut2) == F,
               is.na(triglycerides) == F,
               is.na(age) == F,
               is.na(sexe) == F,
               is.na(apoe.reg2) == F,
...
data_tm_triglycerides$triglycerides = scale(log(data_tm_triglycerides$triglycerides))
data_tm_triglycerides$age = data_tm_triglycerides$age/10

y = "taut2"
dt = data_tm_triglycerides
...
data_tc_triglycerides = filter(data2, is.na(tau) == F,
               is.na(triglycerides) == F,
               is.na(age) == F,
               is.na(sexe) == F,
               is.na(apoe.reg2) == F,
...
data_tc_triglycerides$triglycerides = scale(log(data_tc_triglycerides$triglycerides))
data_tc_triglycerides$age = data_tc_triglycerides$age/10

y = "tau"
dt = data_tc_triglycerides
...
data_ptm$hta_med = as.factor(data_ptm$hta_med)
data_ptm$apoe.reg2 = as.factor(data_ptm$apoe.reg2)

labels(data_ptm)  <- c(age = 'Age', sexe_reg = "Sexe", glucose_jeun = "Glucose",apoe.reg2 = "Présence E4", mmse = "MMSE", ch = "Traitement cholesterol", dia = "Traitement Diabete",triglycerides = "Triglycerides",cholesterol_total = "Cholesterol",cholesterol_hdl = "Cholesterol hdl",cholesterol_ldl = "Cholesterol ldl",niveau_etude_reg = "Niveau d'étude",cholesterol_total = "Cholesterol",glucose_jeun = "Glucose",triglycerides = "Triglycerides",hta_med = "Traitement Hypertenseur", imc_reg = "IMC")

tab1 <- tableby(ptaut2~  age + sexe_reg  + cholesterol_total + glucose_jeun + triglycerides + apoe.reg2 + mmse + ch + dia + hta_med + niveau_etude_reg + imc_reg, data=data_ptm)
summary(tab1, text=TRUE)
```
...
data_ptm_triglycerides = filter(data2, is.na(ptaut2) == F,
               is.na(triglycerides) == F,
               is.na(age) == F,
               is.na(sexe) == F,
               is.na(apoe.reg2) == F,
...
data_ptm_triglycerides$triglycerides = scale(log(data_ptm_triglycerides$triglycerides))
data_ptm_triglycerides$age = data_ptm_triglycerides$age/10

y = "ptaut2"
dt = data_ptm_triglycerides
...
data_ptc_triglycerides = filter(data2, is.na(ptau) == F,
               is.na(triglycerides) == F,
               is.na(age) == F,
               is.na(sexe) == F,
               is.na(apoe.reg2) == F, 
...
data_ptc_triglycerides$triglycerides = scale(log(data_ptc_triglycerides$triglycerides))
data_ptc_triglycerides$age = data_ptc_triglycerides$age/10
y = "ptau"
dt = data_ptc_triglycerides
...
```{r}
data2$presence_biomarqueur = data2$presence_biomarqueur <- ifelse(!is.na(data2$glucose_jeun) | !is.na(data2$triglycerides) | !is.na(data2$cholesterol_total), 1, 0)

write_xlsx(data2,"C:/Users/g_boilay/Documents/data2.xlsx")
write.csv(data2,"C:/Users/g_boilay/Documents/data2.csv")
```
