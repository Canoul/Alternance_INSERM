...
data_chol_f <- dplyr::select(data_chol,abeta42, chol,chol2,chol3,imc_reg, sexe, age_med, mmse_med, niveau_etude_reg, apoe.reg2,ch,dia,hta_med,g)
...
fit <- lm(abeta42 ~ bs(chol, df = 5) + sexe + age_med + imc_reg + niveau_etude_reg + apoe.reg2 + ch + dia + hta_med + mmse_med + g, data = data_chol)
...
fit <- lm(abeta42 ~ poly(chol,k) + sexe + age_med + imc_reg + niveau_etude_reg + apoe.reg2 + ch + dia + hta_med + mmse_med + g, data = data_chol)
...
mod = lm(abeta42 ~ chol + age + sexe + niveau_etude_reg 
         + mmse + ch + dia + hta_med + imc_reg + apoe.reg2 + g, data = data_chol)
...
#### Cubique > Quadratique > Linéaire

```{r,echo=FALSE}
anova(m_cub,m_quad, m_ln, test = "Chisq")
```
#### Linéaire par morceau > Linéaire
...
```{r,echo=FALSE}
plot(data_chol$chol, data_chol$abeta42, col='black', xlab = "cholesterol", ylab = "abeta42",main ="Figure avec les 5 modélisations")
lines(newdata$chol, ypred1[, "fit"], lty = 1, col = "red", lwd = 2)
