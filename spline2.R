...
segmented_model2 <- segmented(mod2,seg.Z = ~cholesterol_ldl, psi = 0)
summary(segmented_model2)
AIC(segmented_model2)

### plot

plot(abeta42 ~ cholesterol_ldl, data = data_ldl, col='steelblue',main ="Model 2")

plot(segmented_model2, add=T)
...
segmented_model3 <- segmented(mod3,seg.Z = ~cholesterol_ldl)
summary(segmented_model3)
AIC(segmented_model3)

modt = lm(abeta42 ~ cholesterol_ldl, data = data_ma_0)
segmented_modelt <- segmented(modt,seg.Z = ~cholesterol_ldl)


### plot

plot(abeta42 ~ cholesterol_ldl, data = data_ma_0, col='steelblue',main ="Model 3")
#plot(segmented_modelt, add=T,col = 'green')
plot(segmented_model3, add=T)
...
segmented_model4 <- segmented(mod4,seg.Z = ~cholesterol_ldl, psi = 0)
summary(segmented_model4)
AIC(segmented_model4)

### plot

plot(abeta42 ~ cholesterol_ldl, data = data_ma_1, col='steelblue',main ="Model 4")

plot(segmented_model4, add=T)
